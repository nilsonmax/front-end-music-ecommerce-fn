...
const LoginForm = ({ visible, onClose }) => {
  ...
  const onClickLogin = (e) => {
    e.preventDefault()
    if (!state.userName || !state.password) return setError("userName or password required")
    if (state.userName < 4) return setError("userName incorrect")
    if (state.userName < 8) return setError("password incorrect")
    ...
    dispatch(loginUser(state))
      ...
      .catch((error) => {
        console.log(error)
        let newError = String(error).replace("TypeError: ", "");
        setError(newError)
      })
  }
        ...
        <div class=" bg-white rounded-lg max-w-screen-2xl px-4 md:px-8 mx-auto">
          ...
          <form class="max-w-lg  mx-auto bg-white ">
            <div class="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
              {logeado&& <p>"sesión iniciada exitosamente"</p>}
              <h2 class="text-dark pt-2  text-2xl lg:text-2xl font-bold text-center">Sign In</h2>
              <button onClick={onClose} type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="defaultModal">
                <svg aria-hidden="true" class="w-8 h-8 hover:bg-primary rounded-lg" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
              </button>
            </div>
            ...
            <div class="flex flex-col gap-4 p-4 md:p-8">
              <div>
                ...
                <input onChange={e => { handledChange(e) }} placeholder='userName...' type="text" name="userName" class="border w-full bg-mintcream  text-dark  focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" />
              </div>

              <div className=''>
                <label for="password" class="inline-block text-dark  text-sm sm:text-base mb-2">Password</label>
                <input type="password" placeholder='password...' onChange={e => { handledChange(e) }} name="password" class="border w-full bg-mintcream  text-dark  focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" />
              </div>
              {error && <p className='bg-highlight text-red-500 text-center p-2 rounded-full font-bold'>{error}</p>}
              <button onClick={(e) => onClickLogin(e)} class="block mb-16 bg-primary  hover:bg-tertiary  active:bg-tertiary  focus-visible:ring ring-tertiary  text-white text-sm md:text-base font-semibold text-center rounded-lg outline-none transition duration-100 px-8 py-3">Log in</button>
