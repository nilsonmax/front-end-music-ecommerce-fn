...
export default function Card({ id, name, brand, price, img, description, stock, status, categoryId, categoryName }) {
  ...
  return (
    <StyledCard>
      ...
      <button
        onClick={() => addToCart({ id, name, brand, price, img, description, stock, status, categoryId, categoryName }, qty)}
        class="inline-flex items-center h-8 px-2 text-background transition-primary duration-150 bg-secondary rounded-lg focus:shadow-outline hover:bg-primary col-span-1">
        <span className="">{`➕`}</span>
